...
namespace _10
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (age < throughputAge)
            {
                Console.WriteLine("Игра доступна только с 18 лет! Возвращайтесь, как подростете.");
            }
            else
            {
                for (attemptLimit = 0; tryCount > attemptLimit; tryCount--)
                {
                    Console.Write("Введите логин и пароль от вашего аккаунта. Попыток осталось: " + tryCount + "" +
                        "\nЛогин: ");
                    ...
                    while (choice != "4")
                    {
                        ...
                        switch (choice)
                        {
                            case "1":
                                ...
                                int coinsGot = random.Next(maxTreasure);
                                money += coinsGot;
